...
| **Higgsfield 测试模板** | 公开落地页可得的模板封面/预览视频仅作为**本地临时测试夹具**导入（`scripts/import-test-templates.mjs`，`source='higgsfield-test'`，媒体存 `.data/` 不进 git）；验证完成后必须运行 `scripts/remove-test-templates.mjs` 全部移除并换成自研模板；禁止提交、分发或上线使用 |
