 ...
 * (libx264, yuv420p). Both steps are swappable: a headless-Chromium HTML
 * renderer can replace renderFrameSvg behind the same interface later.
 */
export async function renderVideo(
  ...
  try {
    ...
    for (let i = 0; i < frames; i++) {
      const svg = renderFrameSvg(plan, i);
      const png = new Resvg(svg, { fitTo: { mode: "original" }, font: { loadSystemFonts: true } }).render().asPng();
