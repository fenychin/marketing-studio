...
const manifestPath = join(root, ".data", "hf-test-templates", "manifest.json");

const { initDb, db } = await import("./src/db/index.js");
const { storeArtifact } = await import("./src/storage.js");
const { migrate } = await import("./src/db/schema.js");
const { nowIso, uuid } = await import("./src/config.js");

const dataDir = process.env.STUDIO_DATA_DIR ?? join(root, "apps", ".data"); // 与 config.ts 的 URL 推导一致
...
for (const t of manifest.templates) {
  ...
  if (t.kind !== "video") referenceAssetId = coverAsset.id;

  // 视频模板:预览 mp4 也入库,把 mp4 地址写到 thumbUrl(前端 <video> 分支可播)
  let thumbUrl = coverAsset.url;
    ...
    try {
      ...
      if (videoResp.ok) {
        ...
        if (videoBuffer.length > 10000) {
          ...
          thumbUrl = videoAsset.url;
          referenceAssetId = videoAsset.id;
        }
      }
    } catch {
      /* 视频预览可选 */
  ...
  await database.run(
    "INSERT INTO templates(id,slug,title,category,kind,credits,aspect_ratio,prompt_template,seed,source,thumb_url,reference_asset_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
    [
      ...
      1, "higgsfield-test", thumbUrl, referenceAssetId,
    ],
  );
  imported += 1;
}
